...
var newType = Types.string
var instance = newType.type
...
var stringTest:Any.Type = String.self
stringTest

(stringTest as! String.Type).init()

stringTest = Int.self
(stringTest as! Int.Type).init()


var memory = UnsafeMutablePointer<UInt8>.allocate(capacity: 2)
memory.initialize(to: 19)
memory.advanced(by: 1).initialize(to: 70)


var value:UInt = UInt(memory.pointee)
value = value << 8

value |= UInt(memory.advanced(by: 1).pointee)
...
//			let type = item.type.Data.pointee
//			print("Type: \(type)")
...
//			}
//			if type == 2 {
//				if let stringValue = String(bytesNoCopy: UnsafeMutableRawPointer(item.value.Data), length: item.value.Length, encoding: .utf8, freeWhenDone: false) {
//					print(stringValue)
//				}
//			}
//		}
